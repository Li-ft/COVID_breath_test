...
def createDF_fromFile(filename):
    fo = open(filename, "r+")
    # print ("Name of the file: ", fo.name)

    dimbatch = -1  # sample dimention
    count = 0  # counter
    block = -1  # new scan
    names = []  # Interval of Scan
    indexes = []  # amu values (same for all scan)
    values = []  # read values
    series = []  # save the sequence to create the dataframe
    lastindex = -1  # check for last element (new scan)

    names.append('amu')  # st fixed name

    for i, line in enumerate(fo):
        line = line.strip()  # ex file jul 9 2021 1_2.ASC contiene le line con uno spazio all'inizio

        if i < 12:  # print file informations
            # print(line)
            continue
        ...
        if line == test:  # white line = end of a scan
            if dimbatch == -1:  # first scan, save the dimention
                dimbatch = count
            ...
            if block == 0:  # first scan: save indexes and values (both series)
                lastindex = indexes[-1]
                series.append(pd.Series(indexes))
                series.append(pd.Series(values))
                values.clear()  # very imp to vlear the values!
            continue

        if "=" in line:  # IntervalofScan line -> save the name
            el = line.split('=')
            # print(el)
            # print(el[0])
            # print(el[-1])
        ...
        line = line.rstrip()
        el = line.split(' ')
        indexes.append(float(el[0]))
        values.append(int(el[-1]))
        count = count + 1
        if float(el[0]) == lastindex:  # end -> last element of a block
            count = 0
            series.append(pd.Series(values))
            values.clear()
    ...
    if '.' in file:
        ...
        if ext == 'ASC':
            ...
            if '_' in namef:
                ...
                ind = new_filename(namef)  # namef.split('_')[0] + '_' + namef.split('_')[1]
                # print(ind)
                ...
                if ind.split('_')[-1] == '3':  # or ind.split('_')[-1] == '1':# NUMBER OF RANGE!!!!
                    ...
                    if count == 0:
                        ...
                        count += 1
                    df_o.loc[ind] = features_o.loc[0]
                    df_n.loc[ind] = features_n.loc[0]
                    # print("added")
        elif ext == 'csv':
